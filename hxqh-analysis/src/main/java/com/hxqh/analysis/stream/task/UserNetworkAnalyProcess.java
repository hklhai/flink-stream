...
import com.hxqh.analysis.stream.map.UserNetworkAnalyMap;
import com.hxqh.analysis.stream.reduce.RealTimeAreaReduce;
import com.hxqh.analysis.stream.reduce.UserNetworkReduce;
import com.hxqh.analysis.stream.sink.RealTimeAreaSink;
import com.hxqh.analysis.stream.sink.UserNetworkSink;
 ...
 * @author Ocean lin
 */
public class UserNetworkAnalyProcess {
    ...
    public static void main(String[] args) {
        ...
        DataStream<UserNetwork> kafkaMessageMap = input.flatMap(new UserNetworkAnalyMap());

        DataStream<UserNetwork> reduce = kafkaMessageMap.keyBy("timestring").countWindow(Long.valueOf(parameterTool.getRequired("windows.size")))
                .reduce(new UserNetworkReduce());
        reduce.addSink(new UserNetworkSink()).name("UserNetworkSink");

        try {
            env.execute("UserNetworkAnalyProcess");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
